...
class Player {
    ...
    getUpgrade(upgradeName,requiredForUpgrade){
        ...
        if(requiredForUpgrade.length==0){
            if(this.upgradesActive.indexOf(upgradeName) == -1){
            alert("well this should help to survive");
            this.upgradesActive.push(upgradeName);
            }else{
                alert("upgrade is already active");
            }
    ...
    upgradeRep(){
        ...
        if(mainCharacter.upgradesActive.length>0){
            for(let item of mainCharacter.upgradesActive){
                summedUpItems.push(`${item} system, `);
            }
    
            console.log(summedUpItems)
    
            return(summedUpItems.toString());
        }else{
             ...
             console.log("positionPlace: ",mainCharacter.givePositionObject(prebuiltLevel));
                ...
                if(userInputKeyboard<5){
                    ...
                    if(userInputKeyboard == 5){
                        ...
                        $('#game-text').append("<p>"+mainCharacter.upgradeRep()+ "</p>");
